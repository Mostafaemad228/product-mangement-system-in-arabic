    ...
    if (title.value != "" && price.value != "" && category.value != "" && count.value <= 100) {
        ...
        if (mood === "Add") {
            ...
            if (newProduct.count > 1) {
                ...
                // }
                count.value = newProduct.count;
                newProduct.total = +total.textContent * +count.value
        ...
        // * if mood is Update make the update process in the product 
            ...
            submit.innerHTML = 'اضافه منتج جديد '
            newProduct.total = +total.textContent * +count.value
...
function showData() {
    ...
    for (let i = 0; i < ProductsData.length; i++) {
        ...
        template += `
            ...
            <td>${ProductsData[i].category}</td>
                ...
                <th id="Update"> <button onclick="UpdateData( ${i} )" class="btn btn-success">  تعديل المنتج </button></th>
                <th id="Delete" onclick="deleteData( ${i} )" ><button class=" btn btn-danger deleteBtn"> مسح المنتج </button></th>
    ...
    if (ProductsData.length > 0) {
        deleteAll.innerHTML = `  <button onclick="deleteAllData()"  class="btn btn-primary col-lg-12 delbtn "   > مسح كل المنتجات  </button> `
    }
    else {
        deleteAll.innerHTML = ""
    }
...
function UpdateData(i) {
    ...
    count.value = ProductsData[i].count;
    getTotalPrice();
...
function searchMood(id) {
    ...
    if (id == 'titleSearch') {
        MoodSearch = 'title'
        search.placeholder = `البحث بواسطه الاسم`
    }

    else {
        MoodSearch = 'category'
        search.placeholder = `البحث بواسطه الفئه`
    }
    search.focus();
    search.value = ""
            ...
            if (ProductsData[i].title.includes(val.toLowerCase())) {
                table += `
                    ...
                    <td>${ProductsData[i].category}</td>
                        ...
                        <td>${ProductsData[i].total}</td>
                        <th id="Update"> <button onclick="UpdateData( ${i} )" class="btn btn-success"> تعديل المنتج  </button></th>
                        <th id="Delete" onclick="deleteData( ${i} )" ><button class=" btn btn-danger "> مسح المنتج  </button></th>
